...
## Player Restriction
- When the player (non-guest) stands at `(4,3)`, the A/S/D/F/G/H/J/K keys do **not** trigger piano notes.
- The piano interaction message from `north-square-piano.spec.md` still applies when walking into `(4,2)`.

## Implementation Notes
- **Input handling** (`src/main.rs`): Inside the guest-enabled branch of the interactive key loop, check if `guest_location == Farm && guest_x == 4 && guest_y == 3`. If true, intercept `KeyCode::Char('a'..'k')` for the 8 mapped keys before falling through to the default "Guest can only walk around." handler.
