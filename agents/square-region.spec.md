...
### Notes
- Bottom opening is at row 4, col 4 (0-based indexing): this is the **Square gate tile**.
- All `🌳` tiles are boundaries and non-walkable.
- The fountain `⛲` at row 2, col 4 is a building and non-walkable.
- The day-1 flower spawn is at row 1, col 1 (`🌺`).
- Grass `🌿` is walkable.
...
### 2) Building Behavior (Fountain)
Fountain (`⛲`) is a **building/obstacle** tile:
- cannot clear,
- cannot plant,
- cannot harvest,
- cannot water,
...
- random daily flower generation is allowed in Square,
- random crop spawn is not allowed in Square,
...
### MCP `print` snapshot
- Must correctly render Square map dimensions (9×5).
- Must display fountain icon near the center.
- Must display day-1 flower spawn at (1,1).
...
1. **Map correctness**
   - Square map is exactly 9×5 with fountain, day-1 flower at (1,1), and bottom-center opening.
...
- Prefer introducing explicit building tile types (`TileType::Fountain`) to avoid overloading existing tile semantics.
